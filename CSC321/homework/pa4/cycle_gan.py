...
def training_loop(dataloader_X, dataloader_Y, test_dataloader_X, test_dataloader_Y, opts):
    ...
    for iteration in range(1, opts.train_iters+1):
        ...
        D_X_loss = criterion(D_X(images_X), utils.to_var(torch.ones(images_X.size(0))))
        D_Y_loss = criterion(D_Y(images_Y), utils.to_var(torch.ones(images_Y.size(0))))
        ...
        D_X_loss = criterion(D_X(fake_X), utils.to_var(torch.zeros(fake_X.size(0))))

        # 4. Generate fake images that look like domain Y based on real images in domain X
        fake_Y = G_XtoY(images_X)

        # 5. Compute the loss for D_Y
        D_Y_loss = criterion(D_Y(fake_Y), utils.to_var(torch.zeros(fake_Y.size(0))))
        ...
        g_loss = criterion(D_X(fake_X), utils.to_var(torch.ones(fake_X.size(0))))

        if opts.use_cycle_consistency_loss:
            reconstructed_Y = G_XtoY(fake_X)
            # 3. Compute the cycle consistency loss (the reconstruction loss)
            cycle_consistency_loss = criterion(reconstructed_Y, images_Y)
        ...
        g_loss = criterion(D_Y(fake_Y), utils.to_var(torch.ones(fake_Y.size(0))))

        if opts.use_cycle_consistency_loss:
            reconstructed_X = G_YtoX(fake_Y)
            # 3. Compute the cycle consistency loss (the reconstruction loss)
            cycle_consistency_loss = criterion(reconstructed_X, images_X)
            g_loss += cycle_consistency_loss
