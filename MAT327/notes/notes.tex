...
\begin{defn*}
    ...
    \begin{itemize}
        ...
        \item \underline{(theorem)} A product of subspaces is a subspace of the product \textit{(subspace and product topology work well)}
        ...
        \begin{itemize}
            ...
            \end{enumerate}
        \end{itemize}
        \item \underline{(theorem)} \textit{subspace and order topology works well if the subspace is convex}
    \end{itemize}
\end{defn*}
...
\begin{defn*}
    ...
    \begin{itemize}
        ...
        \item \underline{(theorem)} Every finite point set, specifically one-point set, in a Hausdorff space is closed
        \item \underline{(theorem)} If $X$ is $T_2$, then a sequence of points of $X$ converges to at most 1 point of $X$
        \item \underline{(theorem)} Every simply ordered set is $T_2$ in the order topology. Product of two $T_2$ space is $T_2$; Subspace of a $T_2$ space is $T_2$ (order/product/subspace topology well behaved with $T_2$)
        \item \textbf{(examples)}
        \begin{itemize}
...
\begin{defn*}
    \textbf{(Uniform Continuity Theorem)} continuous function on compact space is uniformly continuous
    \begin{proof}
...
\begin{defn*}
    \textbf{(Limit Point Compact)} $X$ is limit point compact if every infinite subset of $X$ has a limit point in $X$.
    \begin{itemize}
        \item \underline{(theorem)} Compactness implies limit point compactness, but not conversely (By contrapositive, try to show if $A$ has no limit point it must be finite. Assume $A'=\emptyset$, then $\overline{A}=A$ hence $A$ is closed. Since $X$ compact, $A$ also compact. Since no limit point, can find $U_a \cap A = \{a\}$ for all $a\in A$. Then $\cup_{a\in A} U_a \cup (X-A)$ covers $X$ and emits finite subcover. Since each $U_a$ contains only 1 point of $A$, $A$ must be finite)
        \item (example) Given $Y=\{a,b\}$ with trivial topology. Let $X = \Z_+ \times Y$ is limit point compact but not compact. (limit point compact because any $\{n,a\}$ has a limit point $\{n, b\}$; in fact, every nonempty subset has a limit point in $X$. $X$ not compact since covering by $\{n\} \times Y$ has no subcover)
    \end{itemize}
\end{defn*}
...
\subsubsection{\linkbook{200}{30 The Countability Axioms}}
...
\subsubsection{\linkbook{205}{31 The Separation Axioms}}

\begin{defn*}
    \textbf{($T_0$ Space)} $X$ is $T_0$ if for $x,y\in X$ where $x\neq y$, then either 
    \begin{enumerate}
        \item exists open $U$ such that $x\in U$ and $y\not\in U$ 
        \item exists open $U$ such that $y\in U$ and $x\not\in U$
    \end{enumerate}
\end{defn*}
...
\begin{defn*}
    \textbf{(Separated)} Let $x,y\in X$, $x$ and $y$ can be separated if each lies in a neighborhood which does not contain the other point. (neighborhood not necessarily disjoint)
    \[
        \forall x\neq y\in X \qquad \exists \text{ neighborhoods } U,V \text{ for } x,y \text{ respectively s.t. } y\not\in U \text{ and }x\not\in V
    \]
\end{defn*}
...
\begin{defn*}
    \textbf{($T_1$ Space)} $X$ is $T_1$ if any two distinct points in $X$ are separated
    \begin{itemize}
        \item \underline{(theorem)} A space is $T_1$ if and only if singleton/finite sets are closed
    \end{itemize}
\end{defn*}
...
\subsubsection{\linkbook{210}{32 Normal Spaces}}

\begin{theorem*}
    \textbf{(Every regular space with a countable basis is normal)} $T_3 + \alpha_2 \Rightarrow T_4$
    \begin{proof}
        Let $A,B$ disjoint closed subsets of $X$. For each $x\in A$, exists open neighborhood $U$ s.t. $U\cap B = \emptyset$. By regularity, exists neighborhood $V$ of $x$ such that $\overline{V}\subset U$. Now since there is a countable basis, pick an element from $U_x \in \calB$ such that $x\in U_x\subset V$. Pick such $U_x$ for every $x\in A$, we find a countable covering for $A$, i.e. $\{U_n\}$, such that each $\overline{U}_n \cap B = \emptyset$. Similarly choose $\{V_n\}$ as a covering for $B$ where each $\overline{V}_n \cap A = \emptyset$. But note that $U=\cup_n U_n$ and $V=\cup_n V_n$ not necessarily disjoint. Use trick 
        \[
            U_n' = U_n - \bigcup_{i=1}^n \overline{V}_i 
            \qquad 
            V_n' = V_n - \bigcup_{i=1}^n \overline{U}_i
            \qquad 
            \qquad 
            U' = \bigcup_{n\in \Z_+} U_n'
            \quad
            V' = \bigcup_{n\in \Z_+} V_n'
        \]
        to avoid $U_n$s and $V_n$s from potential intersections. Each $U_n',V_n'$ are open since each is difference of an open set with a closed set. Note $\{U_n'\}$ covers $A$, since any $x\in A$ is in some $U_n$ and in no $\overline{V}_i$. Similarly $\{V'_n\}$ covers $B$. Now we show $U' \cap V' = \emptyset$. Suppose $x\in U' \cap V'$. Then $x\in U_j' \cap V_k'$ for some $j,k$. Suppose $j\leq k$, then by definition of $U_j'$, $x\in U_j$ and by definition of $V_k'$, $x\not\in U_j$. here is a contradiction.
    \end{proof}
\end{theorem*}

\begin{theorem*}
    \textbf{(Every metrizable space is normal)} metrizable $\Rightarrow$ $T_4$
    \begin{proof}
        Given $(X,d)$. Let $A,B\subset X$ disjoint and closed. For each $a\in A$, find $\epsilon_a$ s.t. $B_{\epsilon}(a) \cap B = \emptyset$. Let $U = \cup_{a\in A} B_{\epsilon_a/2}(a)$, similarly for $V=\cup_{b\in B} B_{\epsilon_b/2}(b)$. Note $A\subset U$ and $B\subset V$. Now claim they are disjoint. Suppose $x\in U\cap V$. then $x\in B_{\epsilon_a}(a) \cap B_{\epsilon/b}(b)$ for some $a\in A$ and $b\in B$. Then $d(a,b) \leq d(a,x)+d(x,y) \leq (\epsilon_a+\epsilon_b)/2$, implying either $d(a,b)< \epsilon_a$ or $\epsilon_b$. If former, then $b\in B_{\epsilon_a}(a)$, which is a contradiction. Same for the latter case.
    \end{proof}
\end{theorem*}

\begin{theorem*}
    \textbf{(Compact Hausdorff space is normal)} compact $+ T_2 \Rightarrow T_4$
    \begin{proof}
        Let $A,B$ be disjoint closed subsets of $X$. For each $a\in A$, find $U_a, V_a$ disjoint containing $a$ and $B$ respectively, by regularity of $X$. $\{U_a\}$ covers $A$. Since $A$ compact, $\{U_i \mid i\in I\}$ is a finite cover for $A$. Let $U = \cup_{i\in I} U_i$ and $V = \cap_{i\in I} V_i$ are disjoint open sets containing $A$ and $B$ respectively.
    \end{proof}
\end{theorem*}

\begin{theorem*}
    \textbf{(Every order topology is normal)} order $\Rightarrow T_4$
\end{theorem*}



\subsubsection{\linkbook{216}{33 The Urysohn Lemma}}

\begin{theorem*}
    \textbf{(Urysohn Lemma $T_4 \Rightarrow T_{4.5}$)} Let $X$ be $T_4$; let $A$ and $B$ be disjoint subsets of $X$. There exsits continuous map $f:X\to [a,b]$ such that $f(x) = a$ and $f(B) =\{b\}$
    \begin{proof}
        Proof sketch
        \begin{enumerate}
            \item Construct a simply ordered, countable set $\{U_p \mid p \in [0,1]\cap \Q \}$ such that for all $p<q$, $\overline{U}_p \subset U_q$.
            \item Extend $U_p$ for $p\in \Q$ by defining $U_p = \emptyset$ if $p<0$ and $U_p = X$ if $p>1$
            \item Consider $f(x) = \inf \{ p \mid x\in U_p \}$ and show $f$ is the desired function
            \begin{enumerate}
                \item $x\in A$, then $x\in U_p$ for all $p\geq 0$, hence $f(x) = \inf \{p \in\Q \mid p \geq 0 \} = 0$
                \item $x\in B$, then $x\in U_p$ for no $p \leq 1$, hence $f(x) = \inf \{p\in \Q \mid p > 1 \} = 1$
                \item $f$ continuous
                \begin{itemize}
                    \item (fact) $x\in \overline{U}_r \Rightarrow f(x) \leq r$ (since $x\in U_s$ for all $s>r$, $f(x) = \inf\{p\in \Q \mid p > r\} \leq r$)
                    \item (fact) $x\not\in U_r \Rightarrow f(x)\geq r$ (since $x\not\in U_s$ for any $s<r$, $f(x) = \inf\{p\in\Q \mid p \geq r\} \geq r$)
                \end{itemize}
                Let $x_0\in X$ and $(c,d)\in\R$, want to find neighborhood $U$ of $x_0$ such that $f(U) \in (c,d)$. Pick $p,q\in\Q$ such that $c < p < f(x_0) < q < d$. Let $U = U_q - \overline{U}_p$. Let $y\in U$. By $y\in U_q \in \overline{U}_q$, we have $f(y) \leq q$. By $y\not\in \overline{U}_p$ implying $y\not\in U_p$, we have $f(y) \geq p$. Therefore $f(y) \in [p,q] \subset (c,d)$
            \end{enumerate}
        \end{enumerate}
    \end{proof}
\end{theorem*}
...
\subsubsection{\linkbook{224}{34 The Urysohn Metrization Theorem}}
...
\begin{theorem*}
    \textbf{(Urysohn Metrization Theorem $T_3 + \alpha_2 \Rightarrow $ metric)} Every regular space with a countable basis is metrizable
    \begin{proof}
        Idea is to imbed $X$ in a metrizable space $Y$, thereby proving $X$ is metrizable by homeomorphism. To show $X \hookrightarrow [0,1]^{\omega}$, we need to find a countable collection of continuous functions $\{f_n:X\to [0,1] \mid n\in \Z_+ \}$ that separates points from closed sets. The result follows by applying the imbedding theorem. Let $\{B_n\}$be a countable basis for $X$. Note $X$ is $T_3 + \alpha_2 \Rightarrow T_2 + \alpha_2 \Rightarrow T_4 \Rightarrow T_{4.5}$ by Urysohn lemma. For each $\overline{B}_n \subset B_m$, pick a continuous function $g_{n,m}: X\to [0,1]$ such that $g_{n,m}(\overline{B}_n) = \{1\}$ and $g_{n,m}(X-B_m) = \{0\}$. We claim $\{g_{n,m}\}$ separates points and closed sets! Let $x_0\in X$ and $B\subset X$ closed s.t. $x_0\not\in B$, we can find a basic element $B_m$ of $x$ contained in $B$. Since $X$ is $T_{3.5}$, we can find $B_n$ s.t. $x\in B_n \overline{B}_n \subset B_m$. By construction, $g_{n,m}(x_0) = 1$ and $g_{n,m}(X-U) = \{0\}$. Hence $\{g_{n,m}\}$ as desired. Note $\{g_{n,m}\}$ indexed by $\Z_+ \times \Z_+$ and hence countable. Reindex and this gives us a collection $\{f_{n,m}\}$ which satisfies condition for imbedding theorem.
    \end{proof}
\end{theorem*}

\subsubsection{\linkbook{230}{35 The Tietze Extension Theorem}}

\begin{theorem*}
    \textbf{(Tietze Extension Theorem)} Let $X$ be $T_4$; Let $A\subset X$ be closed. Any continuous function of $A$ into closed $[a,b]\in \R$ ($\R$) maybe extended to a continuous function of all of $X$ into $[a,b]$ ($\R$)
    \[
        X \text{ is } T_4 \;\; A\subset X \text{ closed } \;\; f:A\to \R \text{ continuous }
        \quad 
        \Rightarrow 
        \quad
        \exists \; \tilde{f}: X\to \R \text{ continuous } \;\; \tilde{f}|_A = f
    \]
    \begin{proof}
        Idea is to find $\{g_n\} \to \tilde{f}$ a sequence of functions that uniformly converges to $\tilde{f}$ to ensure its continuity. Let $f:\to [-1,1]$, be continuous, we can use the fact that $X$ is now $T_{4.5}$ (Urysohn lemma) and find $g_1: X\to [-1/3,1/3]$ where $g(B) = -1/3$ and $g(C) = 1/3$ where $B = f^{-1}([-1, -1/3])$ and $C=f^{-1}([1/3,1])$. This function satisfies 
        \begin{align*}
            |g_1(x)| &\leq 1/3  & x\in X \\
            |f(x) - g_1(x)| &\leq 2/3 & x\in A \\
        \end{align*}
        Consider $f-g_1: X\to [-2/3,2/3]$. We can iteratively define $g_n$ and let $\tilde{f}(x) = \sum_{n\in \Z_+} g_n (x)$. $g$ converges and $g$ is uniformly continuous.
    \end{proof}
    \begin{itemize}
        \item \underline{(theorem)} If $X$ satisfies Tietze, then $T$ is $T_{4.5}$ (let $B,C \subset X$ closed. Define $f:B\cup C \to \R$ by $f(x)=0$ if $x\in B$ and $f(1) = 1$ if $x\in C$. $f$ is continuous and use Tietze to find $\tilde{f}:X\to \R$, which is as required for $T_{4.5}$)
    \end{itemize}
\end{theorem*}
...
\subsubsection{\linkbook{240}{37 The Tychonoff Theorem}}
