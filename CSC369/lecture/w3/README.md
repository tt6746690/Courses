...
+ _Scheduling in Interactive System_ 
    + _Round-Robin Scheduling_
        ...
        + _length of quantum_ is a tradeoff 
            ...
            + If quantum is long, 
                ...
                + if quantum is set longer than mean CPU burst, then preemption will not happen very often 
    ...
    + _Quaranteed Scheduling_   
        + make real promises to user about performance a live up to those promises 
        + idea 
...
+ _nice_ 
    ...
    + `nice(value)` syscall, 
        + where `-20 <= value < 19`, 
        + only admin can assign `value` frmo -20 to -1 
+ _runqueue_
    ...
    + _how to determine if a process is I/O or CPU-bound_ 
        ...
        + _dynamic priority_ 
            ...
            + `sleep_avg`
                ...
                + dynamicaly mapped to values in `-5 ~ +5`
...
+ _Completely Fair Scheduler (CFS)_
    ...
    + algorithm
        ...
        + `priority -> effective rate a task's virtual time passes when its on CPU`
            + low priority task, time passes more quickly, their `vruntime` will increase more rapidly, hence will lose CPU and reinserted into CPU sooner than if they had a higher priority value 
