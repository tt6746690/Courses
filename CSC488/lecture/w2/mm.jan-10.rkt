...
#;{(wait! wait?)             ; boolean ‘wait?’ turns on/off pause for input between snapshots
   ...
   ; boolean ‘interleave?’ turns on/off closures interleaved with vs pointing into environment tree.
...
(module model racket
    ...
    (let ([uid 0])
      (λ (parent) (local-require (only-in racket/syntax format-symbol))
        (begin0 (Environment (if (symbol? parent)
                                 '•
                                 (format-symbol "E~a. Call ~a" uid (Tree-uid parent)))
  ...
  (define closure-create!
    (let ([uid 0])
      (λ (name formals body function)
        ...
        (let ([model-closure (Closure (format-symbol "λ~a~a" uid (if name (~a "." name) ""))
  ...
  (define-syntax-rule (wait! sub-forms ...) (~wait! sub-forms ...))
  (define ~wait-declaration! (make-parameter #false))
  (define-syntax-rule (wait-declaration! sub-forms ...) (~wait-declaration! sub-forms ...))
  
  (define (show)
    (when (wait!) (read-line))
    (println (semantic-model global-environment))
    (display "Call Stack: ") (displayln (map Tree-uid call-stack)))
  ...
  (define-syntax-rule (scale! sub-forms ...) (~scale! sub-forms ...))
  (scale! 12)

  (define ~interleave! (make-parameter #true))
  (define-syntax-rule (interleave! sub-forms ...) (~interleave! sub-forms ...))
  ...
  (module Drawing slideshow
    ...
    (define-syntax-rule (body-width! sub-forms ...) (~body-width! sub-forms ...))
    (define inset′ (curryr inset 1 0))
    ...
    (define (boxify picture [shape rectangle])
      (cc-superimpose picture (shape (+ (/ (scale) 2) (pict-width picture))
                                     (+ (/ (scale) 2) (pict-height picture)))))
    (define (layout-siblings siblings) (apply ht-append (scale) siblings))
    (define (layout-parent-children parent children) (vc-append (* 2 (scale)) parent children))
  ...
  (define (value-representation value)
    (define (convert v)
      (cond [(list? v) (map convert v)]
            [(and (procedure? v) (object-name v)) (object-name v)]
            [else v]))
    ((if (Closure? value) closure-name label) (convert value)))
