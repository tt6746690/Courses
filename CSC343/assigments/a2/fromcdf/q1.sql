...
SELECT date_part('year', e_date) as year, election.id as e_id, election_result.id as r_id,
country_id, party_id
FROM Election, Election_result
WHERE election.id = election_id and 1996 <= date_part('year', e_date) and date_part('year', e_date) <= 2016;
