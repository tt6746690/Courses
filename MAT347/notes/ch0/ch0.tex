...
\begin{definition*}
    ...
    \begin{enumerate}
        ...
        \item \bheading{proposition} If $\p{a,b} = d$, then $\p{a/d, b/d} = 1$ (since $d$ contains all common factors of $a$ and $b$)
        ...
        \end{itemize}
        \item \bheading{Euler $\varphi$-function} for $n\in \Z^+$, let $\varphi(n)$ be number of positive integers $a\leq n$ with $a$ relative prime to $n$, i.e. $\p{a,n} = 1$. Then for any prime powers $p^a$ for some $a\geq 1$,
        \[
        ...
        \end{proof}
        \begin{itemize}
            \item \heading{fact} $\varphi$ is multiplicative
            \[
                \varphi(ab) = \varphi(a) \varphi(b)
                \quad \quad \p{a,b}=1    
            \]
            \item \heading{example} $\varphi(12)=\varphi(2^2)\varphi(3) = (12)(1-1/2)(1-1/3) = 4$ (1,5,6,7 are coprime to 12)
            \item \bheading{theorem} $\order{\integermodnmul} = \varphi(n)$ 
        \end{itemize}
    \end{enumerate}
\end{definition*}
