...
;(define (include-in-all elem a-list)
;  (local [(define (include-element a-list)
;            (append (list elem) a-list))]
;    (map include-element a-list))))
...
(define (without element a-list)
  (local [(define (filter-element item)
            (cond [(equal? element item) (list)]
                  [else (list item)]))]
    (apply append (map filter-element a-list))))
...
; partial design
(check-expect (adjacent (list 1 1)) (local [(define x 1)
                                            (define y 1)]
                                      (list (list  x (- y 1))
                                            (list  x (+ y 1))
                                            (list (- x 1)  y)
...
; partial design
(check-expect (adjacent (list 1 1)) (local [(define x (first (list 1 1)))
                                            (define y (second (list 1 1)))]
                                      (list (list  x (- y 1))
                                            (list  x (+ y 1))
                                            (list (- x 1)  y)
                                            (list (+ x 1)  y))))

; (list-ref l i)  return l[i] 
(define (adjacent spot) 
  (local [(define x (first spot))
          (define y (list-ref spot 1))]
    (list (list  x (- y 1))
          (list  x (+ y 1))
          (list (- x 1)  y)
          (list (+ x 1)  y))))
...
(check-expect (paths (list 1 1)          ; to spot
                     (list (list 1 0)    ; lists of spot 
                           (list 0 1) 
                           (list 1 1))
                     (list 0 -1))        ; from spot
              (list))
...
; outer list -> a single list of paths
; middle list -> lists of (x, y) representing a path
; inner list -> list with 2 elements (x,y) representing a coordinate (spot)
...
(check-expect (paths (list 1 1)        ; to
                     square-of-spots   ; through
                     (list 0 0))       ; from
...
(check-expect (paths (list 1 1)        ; to
              ...
              (local [(define (a-p new-from)
                        (paths (list 1 1)
                               (without (list 0 0) square-of-spots)
                               new-from))]   
                (include-in-all (list 0 0)
                                (append (a-p (list  0 -1))
                                        (a-p (list  0  1))
...
(check-expect (paths (list 1 1)        ; to
              ...
              (local [(define (a-p new-from)
                        (paths (list 1 1)
                               ...
                               new-from))]   
                (include-in-all (list 0 0)
                                (apply append (map a-p (adjacent (list 0 0)))))))
...
(check-expect (paths (list 1 1)        ; to
              ...
              (local [(define (a-p new-from)
                        (paths (list 1 1)
                               (without (list 0 0) square-of-spots)
                               new-from))]
                (cond [(equal? (list 1 1) (list 0 0)) (list (list (list 1 1)))]
                      [(not-in? (list 0 0) square-of-spots) (list)]
                      [else (include-in-all (list 0 0)
                                            (apply append (map a-p (adjacent (list 0 0)))))])))
