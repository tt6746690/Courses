...
\section{chapter 1 Topological Spaces and Continuous Functions}
...
\begin{defn*}
    ...
    \begin{itemize}
        ...
        \item \textbf{(theorem)} A product of subspaces is a subspace of the product \textit{(subspace and product topology work well)}
...
\begin{defn*}
    ...
    \begin{itemize}
        ...
        \item \textbf{(theorem)} Every finite point set, specifically one-point set, in a Hausdorff space is closed
        \item \textbf{(theorem)} If $X$ is $T_2$, then a sequence of points of $X$ converges to at most 1 point of $X$
        \item \textbf{(theorem)} Every simply ordered set is $T_2$ in the order topology. Product of two $T_2$ space is $T_2$; Subspace of a $T_2$ space is $T_2$ (order/product/subspace topology well behaved with $T_2$)
        \item \textbf{(examples)}
        \begin{itemize}
...
\begin{defn*}
    ...
    \begin{itemize}
        \item (theorem) $p$ is a surjective continuous map that is either open or closed, then $p$ is a quotient map
...
\begin{defn}
    ...
    \begin{itemize}
        \item (example) $\R$ not compact, since $\calA = \{(n, n+2) \mid n\in \Z_+\}$ does not have a finite subcover
        \item (example) $X = \{0\} \cup \{1/n \mid n\in \Z_+\} \subset \R$ is compact.
    \end{itemize}
\end{defn}
