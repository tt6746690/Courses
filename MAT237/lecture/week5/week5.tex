...
\begin{defn*}
  ...
  If $S\subseteq \R^n$ then a \textbf{path} in $S$ is any continuous map $\gamma: \R \rightarrow \R^n: [0,1]\rightarrow S$. We say that $S$ is \textbf{path-connected} if for every two points $a,b\in S$ there exists a path $\gamma: [0,1]\rightarrow S$ such that $\gamma(0) = a$ and $\gamma(1) = b$

  \begin{rem}
    path-connected is a weaker statement than connectedness. Any path-connected set is necessarily connected; the converse not always true. Geometrically, a set is path-connected if between any points in the set, we can draw a line between those two points that never leaves the set
  \end{rem}
\end{defn*}
...
\begin{center}
  \begin{tabular}{ c c c }
     ...
     infinite union        & no  i.e. $S = [\frac{1}{n}, 1]$      & no  \\
...
\begin{defn*}
  A set $S\subseteq \R^n$ is \textbf{convex} if for all $a,b\in S$, there exists a path
  \[
    \lambda: [0,1] \to l
  \]
  wherer $l\subseteq S$ is a line and
  $\lambda(0) = a$ and $\lambda(1) = b$
  \begin{rem}
    Convex set is path-connected and thus connected. 
  \end{rem}
\end{defn*}
